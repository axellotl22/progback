...
from app.services.job_service import RunJob, list_jobs, create_job, get_job_by_id, get_job_by_name
...
def kmeans_job(data_frame, name, columns, k_cluster, distance_metric, cluster_count_determination):
    """
    Führt KMeans durch
    :param data_frame:
...
async def job_websocket(web_socket: WebSocket, job_id: int, database: Session = Depends(get_db)):
    ...
    j = RunJob(func=kmeans_job,
               args=[read_json(current_job.json_values), current_job.job_name]
...
async def create_kmeans_job(
    ...
    db_job = create_job(database, user_id=0, job_parameters=job_type.to_json(),
...
async def job_list(job_id: Optional[int] = None, job_name: Optional[str] = None,
    ...
    if job_id is not None:
        db_job = get_job_by_id(database, job_id)
        if db_job is None:
            raise HTTPException(status_code=404, detail="Job nicht gefunden!")
        list_content = [db_job, ]
    elif job_name is not None:
        db_job = get_job_by_name(database, job_name)
        if db_job is None:
            raise HTTPException(status_code=404, detail="Job nicht gefunden!")
        list_content = [db_job, ]
    else:
        list_content = list_jobs(database)
    ...
    if with_values:
        result = [JobResponseFull(job_id=db_job.id,
                                  ...
                                  ) for db_job in list_jobs(database)]
    else:
