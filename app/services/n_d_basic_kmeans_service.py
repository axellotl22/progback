...
def extract_important_features(pca: PCA, n_features: int = 5) -> Dict[str, float]:
    ...
    important_indices = np.argsort(pca.explained_variance_ratio_)[
        ::-1][:n_features]
    return {f"feature_{index}": importance for index,
            importance in zip(important_indices, pca.explained_variance_ratio_[important_indices])}
...
def _perform_nd_kmeans(
    ...
    important_features_mapped = {original_columns[int(
        key.split("_")[1])]: value for key, value in important_features.items()}
